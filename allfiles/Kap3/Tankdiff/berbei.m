function [ber,bei ] = berbei(x)
%=======================================================
% The function berbei computes the Kelvin functions
% ber(x) and bei(x) for all values of x.
% -------------- Reference -----------------------------
...
%=======================================================
%
persistent a b Irn Ipn Rrn Rpn;
...
u = x/sqrt(2.0); fac = 1.0/sqrt(2.0*pi*x);
fac1 = cos(3*pi/8 + u); fac2 = sin(3*pi/8 + u);
fac3 = cos(pi/8 + u); fac4 = sin(pi/8 + u);
% ======= ber(x) for abs(x) > 8.0 =======
temp1 = exp(u)*(fac1*SIrn + fac2*SRrn);
temp2 = exp(-u)*(fac3*SIpn + fac4*SRpn);
ber  = fac*(temp1 + temp2);
% ======= bei(x) for abs(x) > 8.0 =======
temp1 = exp(u)*(fac2*SIrn - fac1*SRrn);
temp2 = exp(-u)*(fac3*SRpn - fac4*SIpn);
bei  = fac*(temp1 + temp2);
%
% =============== CHEBY =================
