function[t,y] = rkn2(odefun,tspan,y0,h,method,varargin)
...
switch method
    case {'rk1'}
        ...
        if nlast > 0
            yvec = yvec + hlast*feval(odefun,tt,yvec,varargin{:});
            y(n+2,:) = yvec';
            t(n+2) = tt + hlast;
        end
%         
    case{'rk2GRalston','rk2GMidpoint','rk2'}
        ...
        for k = 1: n
            ...
            k1 = feval(odefun,tt,yvec,varargin{:});
            k2 = feval(odefun,tt+p1*h,yvec+p1*k1*h,varargin{:});
    ...
    case {'rk4'}
        ...
        if nlast > 0
            ...
            y3 = yvec + h2*feval(odefun,tt + h2,y2,varargin{:}); % 2.stage
            y2 = y2 + 2.0*y3;
            y3 = yvec + hlast*feval(odefun,tt + h2,y3,varargin{:}); % 3.stage
            y2 = y2 + y3;
            yvec = (y2 - yvec + h2*feval(odefun,tt + hlast,y3,varargin{:}))/3; % 4.stage
            tt = tt + hlast;
            y(n+2,:) = yvec';
            t(n+2) = tt;
        end
    case{'kutta4'}
        for k= 1:n
            k1 = feval(odefun,tt,yvec,varargin{:}); 
            k2 = feval(odefun,tt+h/3,yvec+k1*h/3,varargin{:}); 
            k3 = feval(odefun,tt+2*h/3,yvec-k1*h/3+h*k2,varargin{:});
            k4 = feval(odefun,tt+h,yvec+k1*h-h*k2+h*k3,varargin{:});
