% ========================= bladelta ==========================
%
% bladelta computes the solution of the Blasius
% equation using finite differences (delta-version):
%             f''' + f*f'' = 0
...
b = zeros(neq,1); a = b; c = b ; d = b; dy = b;
fprintf('        ****************************************\n');
...
while (it <= itmax)& (merror > epsi)
   ...
   b(1) = 2*h*(y(2) - 4*y(1)) - 1;
   b(neq) = 2*h*(y(neq-1) - 2*y(neq)) + 2*h^2; 
   d(1) = 2*h*y(1)*(y(2) - 2*y(1));
   b(neq) = 2*h*(y(neq-1) - 2*y(neq)) + 2*h^2 - 1;
   c(neq-1) = 2*h*y(neq-1)-1;
   d(neq-1) = y(neq) + y(neq-3) - h ...
             -2*(y(neq-2) + h*y(neq-1)*(y(neq) - 2*y(neq-1) + y(neq-2)));
   d(neq) = y(neq)-2*y(neq-1) + y(neq-2) -2*h*y(neq)*(y(neq-1) - y(neq));
            
   dy = penta(e,a,b,c,f,d); % Solve system of equations
   y = y + dy; % Update the y-vector
   merror = sum(abs(dy))/neq; %Compute mean error
   fprintf('     %10.0f   %12.3e  \n',it, merror);
end
if it >= itmax
